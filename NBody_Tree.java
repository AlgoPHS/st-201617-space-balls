...
public class NBody_Tree {
	...
	private class Node {
		...
		// The key used is a square object where the node value is a planet
		// object
		...
		public void drawQuad() {
			...
			//draws quadrant for bebugging purposes
			 StdDraw.square((space.getX()[0] + space.getX()[1]) / 2,
			 (space.getY()[0] + space.getY()[1]) / 2,
			 space.getLen() / 2);

			 //draws celestial bodies as points
		...
		public void initChildren() {
			...
			ur = new Node(new Square((space.getX()[0] + space.getX()[1]) / 2, (space.getY()[0] + space.getY()[1]) / 2,
					space.getLen() / 2));
	...
	// traverses the tree starting at the root to place a planet in it's
	// quadrant
	...
	// recursively goes through the tree to ensure the planet is stored within
	// its own quadrant
	private void add(Node n, Planet b) {

		if (isInternalNode(n)) {
			n.info.updateCenterOfMass(b);
			if (b.isIn(n.ur.space)) {
				add(n.ur, b);
			}
			if (b.isIn(n.ul.space)) {
				add(n.ul, b);

			}
			if (b.isIn(n.dr.space)) {

				add(n.dr, b);
			}
			if (b.isIn(n.dl.space)) {
